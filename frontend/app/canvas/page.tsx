...
import initCanvasKit from "./init";
import initCanvasFonts from "./initFonts";
...
function trackTransforms(ctx: Canvas) {
...
const findWireIndex = (mouseX: number, mouseY: number) => {
  const converted = toCanvasCoords(mouseX, mouseY);
  const x = converted.x;
  const y = converted.y;
...
export default function Home() {
  ...
  const deleteNodeRef = useRef<HTMLButtonElement>(null);
  const wireNodeRef = useRef<HTMLButtonElement>(null);
  const [actionNode, setActioneNode] = useState({
    x: 0,
    y: 0,
    visible: false
  })
  ...
  useEffect(() => {
    ...
    if(!canvasElement) throw ('no canvas');
    Promise.all([initCanvasKit(), initCanvasFonts()]).then(values => {
      const CanvasKit = values[0];
      const fontData = values[1];
      const surface = CanvasKit.MakeWebGLCanvasSurface(canvasElement);
      if(!surface) {
        throw 'cant create surface'
      }
      ...
      function addWire(rect1: NodeR, mouseX: number, mouseY: number) {
        if(!canvasElement) throw ('no canvas');
        const converted = toCanvasCoords(mouseX, mouseY);
        const x = converted.x - canvasElement.offsetLeft;
        const y = converted.y - canvasElement.offsetTop;
      ...
      deleteNodeRef.current.onclick = (event) => {
      ...
      if(wireNodeRef.current)
      wireNodeRef.current.onclick = (event) => {
        if(!selectedRect) return
        closeActionMenu();
        isWiring = true;
      ...
      canvasElement.addEventListener('mouseup', (event) => {
        ...
        console.log(rectangles,wires)
        isDragging = false;
        if(isWiring && selectedRect && selectedRectPrev && selectedWire){
  ...
  const closeActionMenu = () => {
    setActioneNode(prev  => ({...prev, visible: false}))
  }
  const openActionMenu = (x: number, y: number) => {
    isDragging = false;
    setActioneNode({x, y, visible: true})
  }
  return (
    <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
      <canvas ref={canvasRef} id="canvas" width="100px" height="100px"></canvas>
      <div id="node_actions" style={{
        display: actionNode.visible ? 'flex' : 'none',
        position: 'absolute',
        width: 200,
        height: 400,
        top: actionNode.y,
        ...
        <button ref={deleteNodeRef}>delete</button>
        <button ref={wireNodeRef}>wire</button>
      </div>
    </main>
  );
}
